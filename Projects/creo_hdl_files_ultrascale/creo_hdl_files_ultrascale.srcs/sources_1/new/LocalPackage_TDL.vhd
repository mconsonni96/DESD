...
package LocalPackage_TDL is
	...
	--! \brief This function is used in the *AXI4Stream_XUS_VirtualTDLWrapper* module in order to extract from the matrix of delays (*CO_DELAY* \ *O_DELAY*) just the an array of *MAX_NUMBER_OF_TAP* length,
...
package body LocalPackage_TDL is
	...
	---------- Sampling properly the TDL -------------
	--! Description of the signals of the procedure:
	--! \param type_tdl CO vs O vs CO+O Sampling
	--! \param num_tap_tdl Bit of the TDL (number of buffers in the TDL)
	...
	begin
		...
		-- Choose TDL between CO and O Taps of CARRY in Xilinx UltraScale
		if type_tdl = "C" then
