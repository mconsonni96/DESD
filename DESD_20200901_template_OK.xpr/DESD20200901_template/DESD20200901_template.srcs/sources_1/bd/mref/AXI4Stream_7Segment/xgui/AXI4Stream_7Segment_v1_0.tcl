# Definitional proc to organize widgets for parameters.
proc init_gui { IPINST } {
  ipgui::add_param $IPINST -name "Component_Name"
  #Adding Page
  set Page_0 [ipgui::add_page $IPINST -name "Page 0"]
  ipgui::add_param $IPINST -name "ANODE_TICKS" -parent ${Page_0}
  ipgui::add_param $IPINST -name "NUM_OF_DIGITS" -parent ${Page_0}


}

proc update_PARAM_VALUE.ANODE_TICKS { PARAM_VALUE.ANODE_TICKS } {
	# Procedure called to update ANODE_TICKS when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.ANODE_TICKS { PARAM_VALUE.ANODE_TICKS } {
	# Procedure called to validate ANODE_TICKS
	return true
}

proc update_PARAM_VALUE.NUM_OF_DIGITS { PARAM_VALUE.NUM_OF_DIGITS } {
	# Procedure called to update NUM_OF_DIGITS when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.NUM_OF_DIGITS { PARAM_VALUE.NUM_OF_DIGITS } {
	# Procedure called to validate NUM_OF_DIGITS
	return true
}


proc update_MODELPARAM_VALUE.NUM_OF_DIGITS { MODELPARAM_VALUE.NUM_OF_DIGITS PARAM_VALUE.NUM_OF_DIGITS } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.NUM_OF_DIGITS}] ${MODELPARAM_VALUE.NUM_OF_DIGITS}
}

proc update_MODELPARAM_VALUE.ANODE_TICKS { MODELPARAM_VALUE.ANODE_TICKS PARAM_VALUE.ANODE_TICKS } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.ANODE_TICKS}] ${MODELPARAM_VALUE.ANODE_TICKS}
}

