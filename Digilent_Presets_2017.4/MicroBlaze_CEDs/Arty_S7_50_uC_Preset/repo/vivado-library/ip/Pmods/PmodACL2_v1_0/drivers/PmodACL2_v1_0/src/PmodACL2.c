/************************************************************************/
/*																		*/
/* PmodACL2.c  --  Driver source for the Pmod ACL2						*/
/*																		*/
/************************************************************************/
/*	Author:		Mikel Skreen											*/
...
void ACL2_begin(PmodACL2* InstancePtr, u32 GPIO_Address, u32 SPI_Address)
{
	...
	Xil_Out32(InstancePtr->GPIO_addr+ACL2_GPIO_CS_CHANNEL_TRIS_OFFSET, 0b0);
	Xil_Out32(InstancePtr->GPIO_addr+ACL2_GPIO_CS_CHANNEL_DATA_OFFSET, 0b1);
	Xil_Out32(InstancePtr->GPIO_addr+ACL2_GPIO_INT_CHANNEL_TRIS_OFFSET, 0b0011);//0b1111 for input 0b0000 for output, 0b0001 for pin1 in pin 2 out etc.
...
XStatus ACL2_SPIInit(XSpi *SpiInstancePtr)
{
	...
	Status = XSpi_SetOptions(
			SpiInstancePtr,
			XSP_MASTER_OPTION | XSP_CLK_ACTIVE_LOW_OPTION | XSP_CLK_PHASE_1_OPTION | XSP_MANUAL_SSELECT_OPTION
	);
	if (Status != XST_SUCCESS)
	{
		return XST_FAILURE;
...
void ACL2_WriteSpi(PmodACL2* InstancePtr, u8 reg, u8 *wData, int nData)
{
	...
	Xil_Out32(InstancePtr->GPIO_addr+ACL2_GPIO_CS_CHANNEL_DATA_OFFSET, 0b0);
...
**
**	Return Value:
**		the contents of the reg register and next nData-1 consecutive registers through rData
**
**	Description:
...
**
**		We are bit-banging the CS pin of the SPI connection as the manual-mode slave-select
**		in the Xilinx driver appears to not be totally functional.
*/
void ACL2_ReadSpi(PmodACL2* InstancePtr, u8 reg, u8 *rData, int nData)
{
	u8 bytearray[nData+2];
...
**
**	Parameters:
**		buf: results of a SPI read operation from the ACL2, typically a sample from TEMP, X, Y, or Z registers
**
**	Return Value:
**		integer
**
**	Description:
**		Converts an 11 bit sign-extended register-pair reading from the ACL2 into an integer
*/
int ACL2_RawToInt(u8 buf[2])
{
	u16 data;
...
ACL2_SamplePacket ACL2_getSample(PmodACL2 *InstancePtr)
{
	...
	ACL2_ReadSpi(InstancePtr, ACL2_XDATA_L_REG, buf, 6); // all data registers are contiguous in the ACL2 memory space, starting at XDATA_L
...
**
**  Description:
**   	retrieves the 11 bit x, y, z, or temperature values that come from 2 consecutive data registers
*/
int ACL2_getData(PmodACL2 *InstancePtr, u8 start_reg)
{
	u8 buffer[2] = {0,0};

	//store XData high into result //store XData low into 8 LSB
...
**  Parameters:
**  	InstancePtr:	the PmodACL2 device
**		data:			ACL2 reading in units of LSB as determined by ACL2 configuration
**
**  Return Value:
...
float ACL2_DataToG(PmodACL2 *InstancePtr, int data)
{
	...
	{
	case 2: return ((float)data) / 1000.0;
	case 4: return ((float)data) / 500.0;
	case 8: return ((float)data) / 250.0;
	default: return 0.0;
	}
}
