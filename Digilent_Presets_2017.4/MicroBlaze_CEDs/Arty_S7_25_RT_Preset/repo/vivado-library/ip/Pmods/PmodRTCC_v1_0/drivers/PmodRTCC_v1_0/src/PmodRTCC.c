...
**            ** RTCC_TARGET_PWRU and RTCC_TARGET_PWRD does not have the second parameter
**	Return Value:
...
**
**	Description:
**		This function sets the second register of the cheosen target with the value.
*/
void RTCC_setSec(PmodRTCC *InstancePtr, RTCC_Target dest, u8 value)
{
	u8 rgbSec[0];
...
/** void RTCC_setHour12(PmodRTCC *InstancePtr, RTCC_Target dest, u8 value, u8 ampm)
**
**	Parameters:
**		InstancePtr - PmodRTCC device to use
**		dest    - RTCC_TARGET_RTCC - real-time clock
...
**
**	Description:
**		This function returns the state of the VBATEN control bit, which determines where the RTCC draws it's power from.
*/
u8 RTCC_checkVbat(PmodRTCC *InstancePtr)
{
	u8 data;
