...
**
**  Parameters:
**      PmodACL *InstancePtr    - the PmodACL device to communicate with
...
**
**      Description:
**          This function sets the value of some bits (corresponding to the bMask) of a
**          register (indicated by bRegisterAddress) to 1 or 0 (indicated by fValue).
*/
void ACL_SetRegisterBits(PmodACL* InstancePtr, u8 reg, u8 mask, u8 fValue)
{
    u8 regval;
...
**          uiReading   - the 2 bytes containing the reading (in fact only 10 bits are used).
**
**      Return Value:
**          the value of the acceleration in "g" corresponding to the 10 bits reading and the current g range
**
**      Description:
**          Converts the value from the 10 bit reading to the float value (in g) corresponding to
**          the acceleration, considering the current selected g range.
*/
float ConvertReadingToValueG(PmodACL* InstancePtr, int16_t uiReading)
{
    ...
    // m_dGRangeLSB is pre-computed in ACLACL_SetGRange
...
**              For example ACL_PAR_GRANGE_PM8G: Range is 16g, accelerometer is on 10 bits, that corresponds to 16/(2^10).
**              This constant is later used in converting readings to acceleration values in g.
**              (for example converts ACL_PAR_GRANGE_PM8G into 8).
*/
float ACL_GetGRangeLSB(u8 bGRange)
{
    float dGMaxValue = 0;
...
/* ------------------------------------------------------------ */
/***    void ACL_ReadAccelG(PmodACL* InstancePtr, float *dAclXg, float *dAclYg, float *dAclZg)
**
**      Parameters:
**          PmodACL *InstancePtr    - the PmodACL device to communicate with
...
**      Parameters:
**          InstancePtr - the PmodACL device to communicate with
**          iAclX   - the output parameter that will receive acceleration on X axis - 10 bits signed value
**          iAclY   - the output parameter that will receive acceleration on Y axis - 10 bits signed value
**          iAclZ   - the output parameter that will receive acceleration on Z axis - 10 bits signed value
**
**      Return Value:
**          None
**
...
void ACL_SetMeasure(PmodACL* InstancePtr, u8 fMeasure)
{
    ACL_SetRegisterBits(InstancePtr, ACL_REG_POWER_CTL, ACL_MSK_POWER_CTL_MEASURE, fMeasure);
}
...
u8 ACL_GetMeasure(PmodACL* InstancePtr)
{
    return (ACL_GetRegisterBits(InstancePtr, ACL_REG_POWER_CTL, ACL_MSK_POWER_CTL_MEASURE) != 0);
}
...
**      Parameters:
**          InstancePtr - the PmodACL device to communicate with
**          bGRangePar  - the parameter specifying the g range. Can be one of the parameters from the following list:
**              0   ACL_PAR_GRANGE_PM2G Parameter g range : +/- 2g
**              1   ACL_PAR_GRANGE_PM4G Parameter g range : +/- 4g
**              2   ACL_PAR_GRANGE_PM8G Parameter g range : +/- 8g
**              3   ACL_PAR_GRANGE_PM16G Parameter g range : +/- 16g
...
**
**      Description:
**          The function sets the appropriate g range bits in the DATA_FORMAT register.
**              The accepted argument values are between 0 and 3.
**          If the argument is within the accepted values range, it sets the g range bits in DATA_FORMAT
**              register and ACL_ERR_SUCCESS status is returned.
...
void ACL_SetGRange(PmodACL *InstancePtr, u8 bGRangePar)
{
    ...
    ACL_SetRegisterBits(InstancePtr, ACL_REG_DATA_FORMAT, ACL_MSK_DATA_FORMAT_RANGE0, (bGRangePar & 1));
    ACL_SetRegisterBits(InstancePtr, ACL_REG_DATA_FORMAT, ACL_MSK_DATA_FORMAT_RANGE1, (bGRangePar & 2) >> 1);
}
...
**
**      Return Value:
**          The value specifying the g Range parameter. Can be one of the values from the following list
**              0   ACL_PAR_GRANGE_PM2G Parameter g range : +/- 2g
**              1   ACL_PAR_GRANGE_PM4G Parameter g range : +/- 4g
**              2   ACL_PAR_GRANGE_PM8G Parameter g range : +/- 8g
**              3   ACL_PAR_GRANGE_PM16G Parameter g range : +/- 16g
...
u8 ACL_GetGRange(PmodACL* InstancePtr)
{
    return ACL_GetRegisterBits(InstancePtr, ACL_REG_DATA_FORMAT, ACL_MSK_DATA_FORMAT_RANGE1 | ACL_MSK_DATA_FORMAT_RANGE0);
}
...
**      Parameters:
**          InstancePtr - the PmodACL device to communicate with
**          bAxisParam - byte indicating the axis whose offset will be set. Can be one of:
...
**
**      Description:
**          This function sets the specified axis offset, the value being given in “g”.
**              The accepted argument values are between -2g and +2g.
**          If argument is within the accepted values range, its value is quantified in the 8-bit
**              offset register using a scale factor of 15.6 mg/LSB and ACL_ERR_SUCCESS is returned.
**          If value is outside this range or if bAxisParam parameter is outside 0 - 3 range,
...
**      Parameters:
**          InstancePtr - the PmodACL device to communicate with
**          bAxisParam - byte indicating the axis whose acceleration will be read. Can be one of:
**              ACL_PAR_AXIS_X      - indicating X axis
**              ACL_PAR_AXIS_Y      - indicating Y axis
**              ACL_PAR_AXIS_Z      - indicating Z axis
**
...
**          It converts the 8-bit value quantified in the offset register into
**              a value expressed in “g”, using a scale factor of 15.6 mg/LSB.
*/
float ACL_GetOffsetG(PmodACL* InstancePtr, u8 bAxisParam)
{
    int8_t bOffsetVal;
...
**      Parameters:
**          InstancePtr - the PmodACL device to communicate with
**          bAxisInfo - Parameter specifying axes orientation. Can be one of the following:
**              0   ACL_PAR_AXIS_XP - X axis is oriented in the gravitational direction
**              1   ACL_PAR_AXIS_XN - X axis is oriented in the opposite gravitational direction
**              2   ACL_PAR_AXIS_YP - Y axis is oriented in the gravitational direction
**              3   ACL_PAR_AXIS_YN - Y axis is oriented in the opposite gravitational direction
**              4   ACL_PAR_AXIS_ZP - Z axis is oriented in the gravitational direction
**              5   ACL_PAR_AXIS_ZN - Z axis is oriented in the opposite gravitational direction
**
**      Return Value:
**          None
**
